...
from maxent_irl.maxent_irl import (
    CMaxEntIRL, compute_relative_freq, cal_reward_error, cal_policy_error)
...
class ToyMDP(HRMDP):
    ...
    def init_statespace(self):
        ...
        sDrug : state vars associated with drug dosage
        sCannula : state vars associated with cannula removal
        sPatient : state vars associated with patient status
        sSignal : state vars associated with bio-signal
        s = [sDrug, sCannula, sPatient, sSignal]
    ...
    def legal_actions(self, state_idx):
        # print(self.np_idx_to_state)
        # print(self.np_idx_to_state.shape)
        # print(self.np_idx_to_state[state_idx][0])
        sid, = self.np_idx_to_state[state_idx]
        # print(sid)
        state = self.s_space.idx_to_state[sid]
        action_to_idx = lambda act: (
            self.np_action_to_idx[int(self.a_space.action_to_idx[act])])
    ...
    def T(self, state_idx, action_idx):
        ...
        state_to_idx = lambda sttx: (
            self.np_state_to_idx[int(self.s_space.state_to_idx[sttx])])
        ...
        if stt == (0, 0, 1):
            if act == (0, 0):
                list_next_p_state.append((1., TERMINAL_STATE))
            else:
    ...
    def reward(self, state_idx, action_idx, next_state_idx):
        sid, = self.np_idx_to_state[state_idx]
    ...
    def is_terminal(self, state_idx):
        return state_idx == TERMINAL_STATE

################################################################################
# Codes in this section are retrieved from COMP540 assignment 
def q_value(mdp, s_idx, a_idx, U, gamma=0.9):
    np_p_state = mdp.T(s_idx, a_idx)
    if len(np_p_state) == 0:  # illegal action
        return -np.inf

    res = 0
    for p, s_prime_idx in np_p_state:
        s_prime_idx = int(s_prime_idx)
        next_value = 0
        if not mdp.is_terminal(s_prime_idx):
            next_value = U[s_prime_idx]

        res += p * (
            mdp.reward(s_idx, a_idx, s_prime_idx) + gamma * next_value)
    ...
    while True:
        ...
        for s_idx in range(mdp.num_states):
            if mdp.is_terminal(s_idx):
                ...
                U1[s_idx] = max(
                    q_value(mdp, s_idx, a_idx, U, gamma) 
                    for a_idx in mdp.legal_actions(s_idx))
            delta = max(delta, abs(U1[s_idx] - U[s_idx]))
...
# Policy Iteration
def best_policy(mdp, U, gamma=0.9):
    """Given an MDP and a utility function U, determine the best policy,
    as a mapping from state to action."""

    pi = np.zeros((mdp.num_states,), dtype=np.int16)
    for s_idx in range(mdp.num_states):
        # if mdp.is_terminal(s_idx):
        #     pi[s_idx] = mdp.a_space.action_to_idx[(0, 0)]  # better way to conver action and idx?
        # else:
        pi[s_idx] = max(
            mdp.legal_actions(s_idx),
            key=lambda a_idx: q_value(mdp, s_idx, a_idx, U, gamma))
    return pi
...
def gen_trajectory(mdp, stochastic_pi):
    ...
    while True:
        ...
        # list_actions = []
        # list_prop = []
        # for a_idx in stochastic_pi[cur_s_idx]:
        #     list_actions.append(a_idx)
        #     list_prop.append(stochastic_pi[cur_s_idx, a_idx])

        act = random.choices(
            range(stochastic_pi.shape[1]),
            weights=stochastic_pi[cur_s_idx], k=1)[0]
        trajectory.append((cur_s_idx, act))

        # transition to next state
        np_next_dist = mdp.T(cur_s_idx, act)
        # list_next_states = []
        # list_next_prop = []
        ...
        cur_s_idx = int(random.choices(
            np_next_dist[:, 1], weights=np_next_dist[:, 0], k=1)[0])
...
if __name__ == "__main__":
    ...
    gamma = 0.9
    value_fn = value_iteration(toy_mdp, gamma, epsilon=0.001)
    pi = best_policy(toy_mdp, value_fn, gamma)
    ...
    rel_freq = compute_relative_freq(toy_mdp, trajectories)
    reward_error = []
    policy_error = []
    ...
    irl.do_inverseRL(epsilon=0.001, n_max_run=500, callback_reward_pi=compute_errors)
