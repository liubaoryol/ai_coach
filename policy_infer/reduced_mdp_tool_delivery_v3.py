import numpy as np
from enum import Enum
from pyhrcapi.internals.task_mmdp import HRMDP
from pyhrcapi.utils.mdp_utils import Statespace, Actionspace
import bayesian_team_eval.tooldelivery_v3.tooldelivery_v3_state_action as T3SA
from bayesian_team_eval.internals.exceptions import InvalidTransitionError


class ToolLocNew(Enum):
    '''enum of tool locations'''
    STORAGE = 0
    CABINET = 1
    CN = 2
    SN = 3


class Reduced_MDP_Tool_Delivery_v3(HRMDP):
    def __init__(self):
        self.num_tools = 2
        self.num_x_grid = 5
        self.num_y_grid = 5
        self.cabinet_loc = (2, 4)
        self.storage_loc = (0, 4)
        self.handover_loc = (4, 1)
        self.walls = [(1.5, i) for i in range(self.num_y_grid)]
        self.doors = [(1.5, 1)]
        super().__init__(cache_path=None)
        self.num_agents = 3

    def init_statespace(self):
        '''
        We use a factored structure for the state defined as follows:
        sTools[tool_name] : state vars associated with location of tool_name
        sPatient : state vars associated with patient status
        sCNPos : state vars associated with CN positions
        sAsked : state vars associated with whether SN asked a tool
        s = [sTools[too1], sTools[tool2], ..., sPatient, sCNPos, sAsked]
            these factored statespaces are stored in a dictionary
        '''

        self.dict_factored_statespace = {}

        # self.dict_sTools_space = {}
        # self.dict_sTools_space[T3SA.ToolNames.SCALPEL_S] = Statespace(statespace=ToolLocNew)
        # self.dict_sTools_space[T3SA.ToolNames.SUTURE_S] = Statespace(statespace=ToolLocNew)
        self.sScal_space = Statespace(statespace=ToolLocNew)
        self.sSut_space = Statespace(statespace=ToolLocNew)
        # self.sPatient_space = Statespace(statespace=T3SA.StatePatient)
        self.sCNPos_space = Statespace(statespace=T3SA.generate_CNPos_states(
            self.num_x_grid, self.num_y_grid))
   
        # for i, tool in enumerate(T3SA.ToolNames):
        self.dict_factored_statespace[0] = self.sScal_space
        self.dict_factored_statespace[1] = self.sSut_space
        # self.dict_factored_statespace[2] = self.sPatient_space
        self.dict_factored_statespace[2] = self.sCNPos_space

    def init_actionspace(self):
        '''
        We use a factored representation for the joint action:
        aCN : circulating nurse action
        aSN : scrub nurse action
        aAS : surgeon action
        '''

        self.dict_factored_actionspace = {}

        self.aCN_space = Actionspace(actionspace=T3SA.ActionCN)
        self.aSN_space = Actionspace(actionspace=T3SA.ActionSN)
        # self.aAS_space = Actionspace(actionspace=T3SA.ActionAS)

        self.dict_factored_actionspace = {
            0: self.aCN_space, 1: self.aSN_space}

    def legal_actions(self, state_idx):
        state_vector = self.np_idx_to_state[state_idx]
        sScal_s, sSut_s, sCNPos = state_vector 

        list_acts = []
        for action_idx in range(self.num_actions):
            action_vector = self.np_idx_to_action[action_idx]
            aCN, aSN = action_vector

            try:
                self._raise_exception_if_illegal_T(
                    sScal_s, sSut_s, sCNPos, aCN, aSN)
                list_acts.append(action_idx)
            except InvalidTransitionError:
                continue

        return list_acts

    def T(self, state_idx, action_idx):
        '''
        Returns a np array with two columns and at least one row.
        The first column corresponds to the probability for the next state.
        The second column corresponds to the index of the next state.
        '''

        ##! unpack the input
        state_vector = self.np_idx_to_state[state_idx]
        sScal_s, sSut_s, sCNPos = state_vector 

        action_vector = self.np_idx_to_action[action_idx]
        aCN, aSN = action_vector

        self._raise_exception_if_illegal_T(
            sScal_s, sSut_s, sCNPos, aCN, aSN)

        np_next_p_sScal_s = self._T_sScalpel(
            sTool=sScal_s, sCNPos=sCNPos, aCN=aCN, aSN=aSN)
        np_next_p_sSut_s = self._T_sSuture(
            sTool=sSut_s, sCNPos=sCNPos, aCN=aCN, aSN=aSN)

        # deterministic
        np_next_p_sCNPos = self._T_sCNPos(sCNPos=sCNPos, aCN=aCN)
        sPos_prime = np_next_p_sCNPos[0][1].astype(np.int32)

        ##! Merge np_next_p_s* to get np_next_p_state_idx
        dict_next_p_state_idx = {}
        for p_ScS, sScS in np_next_p_sScal_s:
            for p_SuS, sSuS in np_next_p_sSut_s:
                next_state_idx = self.np_state_to_idx[
                    sScS.astype(np.int32), sSuS.astype(np.int32), sPos_prime]
                next_p = p_ScS * p_SuS
                dict_next_p_state_idx[next_state_idx] = next_p

        num_next_states = len(dict_next_p_state_idx)
        np_next_p_state_idx = np.zeros((num_next_states, 2))
        iter_idx = 0
        for next_state_idx in dict_next_p_state_idx:
            next_p = dict_next_p_state_idx.get(next_state_idx)
            np_next_p_state_idx[iter_idx] = next_p, next_state_idx
            iter_idx += 1

        # assert( np.sum(np_next_p_state_idx[:,0]) == 1. )

        return np_next_p_state_idx


    def _raise_exception_if_illegal_T(
        self, sScal_s, sSut_s, sCNPos, aCN, aSN):
        coord_cn = self.sCNPos_space.idx_to_state[sCNPos]
        act_cn = self.aCN_space.idx_to_action[aCN]
        act_sn = self.aSN_space.idx_to_action[aSN]

        scalpel_s_loc = self.sScal_space.idx_to_state[sScal_s]
        suture_s_loc = self.sSut_space.idx_to_state[sSut_s]
        list_spare_tool_loc = [scalpel_s_loc, suture_s_loc]

        if (
            (coord_cn not in [self.cabinet_loc, self.storage_loc]) and
            act_cn == T3SA.ActionCN.PICKUP
            ):
            raise InvalidTransitionError
        
        if coord_cn != self.handover_loc and act_cn == T3SA.ActionCN.HANDOVER:
            raise InvalidTransitionError

        if (
            coord_cn == self.cabinet_loc and
            act_cn == T3SA.ActionCN.PICKUP and
            (ToolLocNew.CABINET not in list_spare_tool_loc)
            ):
            raise InvalidTransitionError

        if (
            coord_cn == self.storage_loc and
            act_cn == T3SA.ActionCN.PICKUP and
            (ToolLocNew.STORAGE not in list_spare_tool_loc)
            ):
            raise InvalidTransitionError
        
        if (
            act_cn == T3SA.ActionCN.HANDOVER and
            (ToolLocNew.CN not in list_spare_tool_loc)
            ):
            raise InvalidTransitionError


    def _T_sScalpel(self, sTool, sCNPos, aCN, aSN):
        tool_loc = self.sScal_space.idx_to_state[sTool]
        coord_cn = self.sCNPos_space.idx_to_state[sCNPos]
        act_cn = self.aCN_space.idx_to_action[aCN]
        act_sn = self.aSN_space.idx_to_action[aSN]

        sTool_cn = (
            self.sScal_space.state_to_idx[ToolLocNew.CN])
        sTool_sn = (
            self.sScal_space.state_to_idx[ToolLocNew.SN])

        if (
            act_cn == T3SA.ActionCN.PICKUP and
            coord_cn == self.cabinet_loc and
            tool_loc == ToolLocNew.CABINET
            ):
            return np.array([[1.0, sTool_cn]])
        elif (
            act_cn == T3SA.ActionCN.PICKUP and
            coord_cn == self.storage_loc and
            tool_loc == ToolLocNew.STORAGE
            ):
            return np.array([[1.0, sTool_cn]])
        elif (
            act_cn == T3SA.ActionCN.HANDOVER and
            coord_cn == self.handover_loc and
            tool_loc == ToolLocNew.CN
            ):
            return np.array([[1.0, sTool_sn]])
        else:
            return np.array([[1.0, sTool]])

    def _T_sSuture(self, sTool, sCNPos, aCN, aSN):
        tool_loc = self.sSut_space.idx_to_state[sTool]
        coord_cn = self.sCNPos_space.idx_to_state[sCNPos]
        act_cn = self.aCN_space.idx_to_action[aCN]
        act_sn = self.aSN_space.idx_to_action[aSN]

        sTool_cn = (
            self.sSut_space.state_to_idx[ToolLocNew.CN])
        sTool_sn = (
            self.sSut_space.state_to_idx[ToolLocNew.SN])

        if (
            act_cn == T3SA.ActionCN.PICKUP and
            coord_cn == self.cabinet_loc and
            tool_loc == ToolLocNew.CABINET
            ):
            return np.array([[1.0, sTool_cn]])
        elif (
            act_cn == T3SA.ActionCN.PICKUP and
            coord_cn == self.storage_loc and
            tool_loc == ToolLocNew.STORAGE
            ):
            return np.array([[1.0, sTool_cn]])
        elif (
            act_cn == T3SA.ActionCN.HANDOVER and
            coord_cn == self.handover_loc and
            tool_loc == ToolLocNew.CN
            ):
            return np.array([[1.0, sTool_sn]])
        else:
            return np.array([[1.0, sTool]])
  
    def _T_sCNPos(self, sCNPos, aCN):
        coord_cn = self.sCNPos_space.idx_to_state[sCNPos]
        action_cn = self.aCN_space.idx_to_action[aCN]

        def bound(coord):
            x, y = coord
            if x < 0:
                x = 0
            elif x >= self.num_x_grid:
                x = self.num_x_grid - 1
            
            if y < 0:
                y = 0
            elif y >= self.num_y_grid:
                y = self.num_y_grid - 1

            return (x, y)

        def is_valid_move(coord, coord_new):
            'assuem sum of coord increment is 0 or 1'
            x_a, y_a = coord
            x_b, y_b = coord_new
            # make a always smaller than or equal to b
            if x_a > x_b:
                x_a, x_b = x_b, x_a
            if y_a > y_b:
                y_a, y_b = y_b, y_a

            for x_d, y_d in self.doors:
                if y_a == y_b and y_a == y_d and x_a < x_d and x_b > x_d:
                    return True
                if x_a == x_b and x_a == x_d and y_a < y_d and y_b > y_d:
                    return True
            
            for x_w, y_w in self.walls:
                if y_a == y_b and y_a == y_w and x_a < x_w and x_b > x_w:
                    return False
                if x_a == x_b and x_a == x_w and y_a < y_w and y_b > y_w:
                    return False
            
            return True

        cn_move_actions = [
            T3SA.ActionCN.MOVE_DOWN, T3SA.ActionCN.MOVE_UP,
            T3SA.ActionCN.MOVE_LEFT, T3SA.ActionCN.MOVE_RIGHT]

        if action_cn not in cn_move_actions:
            return np.array([[1., sCNPos]])
        else:
            new_coord = coord_cn
            if action_cn == T3SA.ActionCN.MOVE_DOWN:
                new_coord = bound((coord_cn[0], coord_cn[1] + 1))
            elif action_cn == T3SA.ActionCN.MOVE_UP:
                new_coord = bound((coord_cn[0], coord_cn[1] - 1))
            elif action_cn == T3SA.ActionCN.MOVE_LEFT:
                new_coord = bound((coord_cn[0] - 1, coord_cn[1]))
            elif action_cn == T3SA.ActionCN.MOVE_RIGHT:
                new_coord = bound((coord_cn[0] + 1, coord_cn[1]))

            if not is_valid_move(coord_cn, new_coord):
                new_coord = coord_cn

            return np.array([[1., self.sCNPos_space.state_to_idx[new_coord]]])

    def is_terminal(self, obstate_idx):
        state_vector = self.np_idx_to_state[int(obstate_idx)]
        sScal_s, sSut_s, sPos = state_vector 

        s_scal_s = self.sScal_space.idx_to_state[sScal_s]
        s_sut_s = self.sSut_space.idx_to_state[sSut_s]

        if (
            s_scal_s == ToolLocNew.SN or
            s_sut_s == ToolLocNew.SN 
            ):
            return True

        return False

if __name__ == "__main__":
    task_mdp = Reduced_MDP_Tool_Delivery_v3()
    sCoord = task_mdp.sCNPos_space.state_to_idx[(1, 3)]
    aCN = task_mdp.aCN_space.action_to_idx[T3SA.ActionCN.MOVE_RIGHT]
    np_new = task_mdp._T_sCNPos(sCoord, aCN)
    sCoord_n = int(np_new[0][1])
    print(task_mdp.sCNPos_space.idx_to_state[sCoord_n])

