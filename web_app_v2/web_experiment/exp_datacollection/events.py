...
from web_experiment.define import ExpType, DATACOL_SESSIONS
...
from web_experiment.exp_datacollection.define import (GAMEPAGES,
                                                      get_socket_namespace,
                                                      get_socket_type)

g_id_2_user_data = {}  # type: Mapping[str, Exp1UserData]

for page_key in DATACOL_SESSIONS:
  name_space = '/' + get_socket_namespace(page_key)

  def make_init_canvas(page_key):
    def initial_canvas():
      ...
      event_impl.initial_canvas(page_key, user_data,
                                GAMEPAGES[get_socket_type(page_key)])
  ...
  def make_button_clicked(page_key):
  ...
  socketio.on_event('connect', make_init_canvas(page_key), name_space)
  socketio.on_event('disconnect', make_disconnected(), name_space)
  socketio.on_event('button_clicked', make_button_clicked(page_key), name_space)
