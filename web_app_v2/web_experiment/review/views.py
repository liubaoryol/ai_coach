from flask import render_template, session, flash, g, request, redirect
...
def replay(session_name, user_id):
  error = load_session_trajectory(session_name, user_id)
  if error:
    flash(error)
...
def record(session_name, user_id):
  ...
  socket_name = get_socket_name(PageKey.Record, get_domain_type(session_name))
...
def review(session_name):
  ...
  error = load_session_trajectory(session_name, cur_user)
  if error:
    flash(error)
