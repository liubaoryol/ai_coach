...
def sample_next_states(head, max_size, ns_idx_offset, batch_idxs, states,
                       ns_buffer):
  ...
  buffer_ns_locs = np.argwhere((head < ns_batch_idxs) & (
      ns_batch_idxs <= head + ns_idx_offset)).flatten()
...
class Replay(Memory):
  ...
  def __init__(self, batch_size, max_size, use_cer):
    super().__init__(batch_size, max_size, use_cer)
    ...
    for k in self.data_keys:
      if k == 'next_states':
        batch[k] = sample_next_states(self.head, self.max_size,
                                      self.ns_idx_offset, self.batch_idxs,
                                      self.states, self.ns_buffer)
      else:
