...
from gym.wrappers.clip_action import ClipAction  # noqa: F401
...
def make_vec_envs(env_name,
  ...
  envs = [
      make_env(env_name, seed, i, log_dir, allow_early_resets, env_make_kwargs)
...
class VecPyTorch(VecEnvWrapper):
  ...
  def step_async(self, actions):
    if isinstance(actions,
                  torch.LongTensor) or (isinstance(actions, torch.Tensor)
                                        and actions.dtype == torch.int64):
