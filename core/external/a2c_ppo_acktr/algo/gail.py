...
class Discriminator(nn.Module):
  ...
  def update(self, expert_loader, rollouts, obsfilt=None):
    ...
    for expert_batch, policy_batch in zip(expert_loader, policy_data_generator):
      ...
      policy_action = conv_discrete_2_onehot(policy_action, self.num_action)

      policy_d = self.trunk(torch.cat([policy_state, policy_action], dim=1))

      expert_state, expert_action = expert_batch
      expert_state = conv_discrete_2_onehot(expert_state, self.num_obs)
      expert_action = conv_discrete_2_onehot(expert_action, self.num_action)
  ...
  def predict_reward(self, state, action, gamma, masks, update_rms=True):
    with torch.no_grad():
      ...
      action = conv_discrete_2_onehot(action, self.num_action)
