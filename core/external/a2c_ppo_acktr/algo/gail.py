...
class Discriminator(nn.Module):
    def __init__(self,
        ...
        self.is_discrete_obs = is_discrete_obs
    ...
    def _conv_input(self, inputs, num_classes):
        if self.is_discrete_obs:
            return F.one_hot(inputs.squeeze(1),
    ...
    def update(self, expert_loader, rollouts, obsfilt=None):
        ...
        for expert_batch, policy_batch in zip(expert_loader,
            ...
            policy_state = self._conv_input(policy_state, self.num_obs)
            policy_action = self._conv_input(policy_action, self.num_action)

            policy_d = self.trunk(
                torch.cat([policy_state, policy_action], dim=1))

            expert_state, expert_action = expert_batch
            expert_state = self._conv_input(expert_state, self.num_obs)
            expert_action = self._conv_input(expert_action, self.num_action)
    ...
    def predict_reward(self, state, action, gamma, masks, update_rms=True):
        with torch.no_grad():
            ...
            state = self._conv_input(state, self.num_obs)
            action = self._conv_input(action, self.num_action)
