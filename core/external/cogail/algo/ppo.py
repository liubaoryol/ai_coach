...
class PPO():
  ...
  def pretrain(self, gail_train_loader, device):
    ...
    for expert_batch in gail_train_loader:
      ...
      expert_action1 = torch.as_tensor(expert_action[:, 0], device=device)
      expert_action2 = torch.as_tensor(expert_action[:, 1], device=device)

      bs = len(expert_state)
      expert_seed = torch.Tensor(
          np.array([[random.randrange(self.actor_critic.num_code)]
                    for _ in range(bs)])).view(bs, 1).to(device)
      ...
      loss1 = self.MSEloss(expert_action1, pred_act[:, 0])
      loss2 = self.MSEloss(expert_action2, pred_act[:, 1])
      loss = loss1 + loss2
    ...
    for e in range(self.ppo_epoch):
      ...
      for expert_batch, sample in zip(expert_loader, data_generator):
        ...
        expert_action_1hot_1 = conv_discrete_2_onehot(
            expert_action[:, 0], self.actor_critic.tuple_num_outputs[0])
