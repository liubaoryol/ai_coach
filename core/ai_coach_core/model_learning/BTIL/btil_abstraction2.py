...
class BTIL_Abstraction2:
  ...
  def forward_backward_messaging(self, idx_a, trajectory, np_abs_tilde,
                                 np_pi_tilde, np_tx_tilde, np_bx_tilde):
    n_lat = self.tuple_num_latents[idx_a]
    # Forward messaging
    with np.errstate(divide='ignore'):
      np_log_forward = np.log(
          np.zeros((len(trajectory), self.num_abstates, n_lat)))

    t = 0
    state_p, joint_a_p, _ = trajectory[t]

    with np.errstate(divide='ignore'):
      np_log_forward[t] = 0.0
      np_log_forward[t] += (np.log(np_abs_tilde[state_p])[:, None] +
                            np.log(np_bx_tilde) +
                            np.log(np_pi_tilde[:, :, joint_a_p[idx_a]]))

    # t = 1:N-1
    for t in range(1, len(trajectory)):
      t_p = t - 1
      state, joint_a, _ = trajectory[t]

      with np.errstate(divide='ignore'):
        np_log_prob = np_log_forward[t_p].reshape(self.num_abstates, n_lat, 1,
                                                  1)

        tx_index = (slice(None), *joint_a_p, slice(None), slice(None))
        np_log_prob = np_log_prob + np.log(np_abs_tilde[state]).reshape(
            1, 1, -1, 1)
        np_log_prob = np_log_prob + np.log(np_tx_tilde[tx_index]).reshape(
            1, n_lat, self.num_abstates, n_lat)
        np_log_prob = np_log_prob + np.log(
            np_pi_tilde[:, :, joint_a[idx_a]]).reshape(1, 1, self.num_abstates,
                                                       n_lat)
    ...
    state_n, joint_a_n, _ = trajectory[t]
    ...
    for t in reversed(range(0, len(trajectory) - 1)):
      ...
      with np.errstate(divide='ignore'):
        np_log_prob = np_log_backward[t_n].reshape(1, 1, self.num_abstates,
                                                   n_lat)

        tx_index = (slice(None), *joint_a, slice(None), slice(None))
        np_log_prob = np_log_prob + np.log(np_abs_tilde[state_n]).reshape(
            1, 1, -1, 1)
        np_log_prob = np_log_prob + np.log(np_tx_tilde[tx_index]).reshape(
            1, n_lat, self.num_abstates, n_lat)
        np_log_prob = np_log_prob + np.log(
            np_pi_tilde[:, :, joint_a_n[idx_a]]).reshape(
                1, 1, self.num_abstates, n_lat)
    ...
    q_z = np.sum(q_zx, axis=2)

    with np.errstate(divide='ignore'):
      log_q_x_xn = np.log(
          np.zeros((len(trajectory) - 1, self.num_abstates, n_lat,
                    self.num_abstates, n_lat)))

    for t in range(len(trajectory) - 1):
      state, joint_a, _ = trajectory[t]
      state_n, joint_a_n, _ = trajectory[t + 1]

      with np.errstate(divide='ignore'):
        log_q_x_xn[t] = (
            np_log_forward[t].reshape(self.num_abstates, n_lat, 1, 1) +
            np_log_backward[t + 1].reshape(1, 1, self.num_abstates, n_lat))
        tx_index = (slice(None), *joint_a, slice(None), slice(None))

        log_q_x_xn[t] += np.log(np_abs_tilde[state_n]).reshape(1, 1, -1, 1)
        log_q_x_xn[t] += np.log(np_tx_tilde[tx_index]).reshape(
            1, n_lat, self.num_abstates, n_lat)
        log_q_x_xn[t] += np.log(np_pi_tilde[:, :, joint_a_n[idx_a]]).reshape(
            1, 1, self.num_abstates, n_lat)

    q_x_xn = softmax(log_q_x_xn, axis=(1, 2, 3, 4))
    q_xzx = np.sum(q_x_xn, axis=0)

    return q_z, q_zx, q_xzx
  ...
  def compute_local_variables(self, idx_a, samples):
    ...
    for m_th in range(len(samples)):
      ...
      # compute q_x, q_xx
      np_q_z, np_q_zx, np_q_xzx = self.forward_backward_messaging(
          idx_a, trajectory, np_abs_tilde, np_pi_tilde, np_tx_tilde,
          np_bx_tilde)

      list_q_z.append(np_q_z)
      list_q_zx.append(np_q_zx)
      list_q_xzx.append(np_q_xzx)

    return list_q_z, list_q_zx, list_q_xzx

  def update_global_variables(self, samples, lr,
                              list_list_q_z: Sequence[Sequence[np.ndarray]],
                              list_list_q_zx: Sequence[Sequence[np.ndarray]],
                              list_list_q_xzx: Sequence[Sequence[np.ndarray]]):
    ...
    for idx_a in range(self.num_agents):
      ...
      for m_th in range(len(samples)):
        ...
        q_zx = list_list_q_zx[idx_a][m_th]
        q_xzx = list_list_q_xzx[idx_a][m_th]

        t = 0
        state, joint_a, _ = traj[t]
        param_abs_hat[state] += q_z[t]
        param_bx_hat += q_zx[t]
        param_pi_hat[:, :, joint_a[idx_a]] += q_zx[t]
        ...
        for t in range(1, len(traj)):
          ...
          param_abs_hat[state] += q_z[t]
          tx_index = (slice(None), *joint_a_p, slice(None), slice(None))
          param_tx_hat[tx_index] += q_xzx[tp]
          param_pi_hat[:, :, joint_a[idx_a]] += q_zx[t]
  ...
  def convert_params_to_prob(self):
    for i_a in range(self.num_agents):
      ...
      self.list_np_policy[i_a] = (numerator /
                                  action_sums[..., np.newaxis]).transpose(
                                      (1, 0, 2))
