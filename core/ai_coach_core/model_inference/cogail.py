...
import external.cogail.utils as cogail_utils
import external.cogail.envs as cogail_env
...
def cogail_w_ppo(mdp: mdp_lib.MDP,
  ...
  env_kwargs = dict(num_states=mdp.num_states,
                    ...
                    cb_legal_actions=mdp.legal_actions,
                    init_state=init_state)

  venv = cogail_env.make_vec_envs('envfrommdp-v0',
                                  seed=args.seed,
  ...
  discriminator = cogail_gail.Discriminator(num_obs=venv.observation_space.n,
                                            tuple_num_actions=tuple(
                                                venv.action_space.nvec),
  ...
  rollouts = cogail_storage.RolloutStorage(
      ...
      venv.random_seed_space.n, venv.action_space,
      actor_critic.recurrent_hidden_state_size)

  # reset env
  obs = venv.reset()
  random_seed = random.randrange(num_latent)
  random_seed = actor_critic._conv_discrete(random_seed, num_latent)
  ...
  for j in tqdm.tqdm(range(num_updates)):
    if args.use_linear_lr_decay:
      cogail_utils.update_linear_schedule(agent.optimizer, j, num_updates,
                                          args.ppo_lr)
    ...
    for _ in range(gail_epoch):
      ...
      #  cogail_utils.get_vec_normalize(venv)._obfilt)
...
def get_np_policy(actor_critic: cogail_model.Policy, num_code: int,
                  num_states: int, num_actions: int, device):
  list_np_policy = [
      np.zeros((num_code, num_states, num_actions)) for _ in range(2)
  ]
  with torch.no_grad():
    for xidx in range(num_code):
      code = torch.Tensor([[xidx]]).long()
      code_1hot = actor_critic._conv_discrete(code, num_code).to(device)
      ...
      for sidx in range(num_states):
        ...
        list_np_policy[0][xidx, sidx, :] = probs1
        list_np_policy[1][xidx, sidx, :] = probs2
