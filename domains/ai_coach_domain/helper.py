...
class TrueModelConverter:
  def __init__(self, agent1: AIAgent_Abstract, agent2: AIAgent_Abstract,
               num_latents) -> None:
    self.agent1 = agent1
    self.agent2 = agent2
  ...
  def get_true_policy(self, agent_idx, latent_idx, state_idx):
    if agent_idx == 0:
      return self.agent1.get_action_distribution(state_idx, latent_idx)
    else:
      return self.agent2.get_action_distribution(state_idx, latent_idx)

  def get_true_Tx_nxsas(self, agent_idx, latent_idx, state_idx,
                        tuple_action_idx, next_state_idx):
    if agent_idx == 0:
      return self.agent1.get_next_latent_distribution(latent_idx, state_idx,
                                                      tuple_action_idx,
                                                      next_state_idx)
    else:
      return self.agent2.get_next_latent_distribution(latent_idx, state_idx,
                                                      tuple_action_idx,
                                                      next_state_idx)

  def get_init_latent_dist(self, agent_idx, state_idx):
    if agent_idx == 0:
      return self.agent1.get_initial_latent_distribution(state_idx)
    else:
      return self.agent2.get_initial_latent_distribution(state_idx)
