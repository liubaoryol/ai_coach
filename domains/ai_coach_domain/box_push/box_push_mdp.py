...
class BoxPushMDP(mdp_lib.MDP):
  ...
  def conv_sim_states_to_mdp_sidx(self, a1_pos, a2_pos, box_states):
    len_s_space = len(self.dict_factored_statespace)
    a1_next_idx = self.a1_pos_space.state_to_idx[a1_pos]
    a2_next_idx = self.a2_pos_space.state_to_idx[a2_pos]
  ...
  def conv_mdp_sidx_to_sim_states(self, state_idx):
    ...
    return a1_pos, a2_pos, box_states
  ...
  def reward(self, state_idx: int, action_idx: int, *args, **kwargs) -> float:
    ...
    both_drop = False
    if (a1_pos in self.goals and a1_pos == a2_pos and act1 == EventType.HOLD
...
if __name__ == "__main__":
  ...
  pi, np_v_value, np_q_value = plan_lib.value_iteration(
      ...
      box_push_mdp.np_reward_model,
      discount_factor=GAMMA,
      max_iteration=100,
      epsilon=0.001)

  with open("box_push_np_v_value.pickle", "wb") as f:
    pickle.dump(np_v_value, f, pickle.HIGHEST_PROTOCOL)
  with open("box_push_np_q_value.pickle", "wb") as f:
  ...
  if QLEARN:
    ...
    NUM_TRAIN = 10000
    ALPHA = 0.5
    box_push_ql = qlearn_lib.QLearningSoftmax(
        beta=1,
    ...
    for i in range(NUM_TRAIN):
      ...
      print("episodes: %d, count: %d, reward: %.2f" %
            (box_push_ql.get_episodes_sofar(), count,
             box_push_ql.episode_rewards))
