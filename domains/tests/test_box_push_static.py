...
def main(gen_trainset, gen_testset, show_true, show_bc, dnn_bc, show_sl,
         ...
         ppo_batch_size, num_iterations, pretrain_steps, use_ce):
  # set simulator
  #############################################################################
  ...
  if show_true:
    ...
    sup_conf_true, full_acc_true = get_bayesian_infer_result(
        num_agents, policy_true, MDP_AGENT.num_latents, test_traj, test_labels)
  ...
  def accuracy_history(num_agent, pi_hyper):
    ...
    conf_full, full_acc = get_bayesian_infer_result(
        num_agent, (lambda m, x, s, joint: list_np_policies[m][x, s, joint[m]]),
        MDP_AGENT.num_latents, test_traj, test_labels)
  ...
  init_state = MDP_AGENT.conv_sim_states_to_mdp_sidx(sim.a1_init, sim.a2_init,
  ...
  if magail:
    ...
    for idx in list_idx:
      ...
      print("MAGAIL %d" % (idx, ))
      print("#########")
      list_pi_magail = magail_by_latent(trajectories[:idx], latent_labels[:idx],
                                        init_state)
